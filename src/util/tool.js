...
let tool = {
    ...
    arrayServer:{
        _private:{
            ...
            getIndexByObject: function (array, obj) {
                let _index = -1;
                for (let i = 0; i < array.length; i++) {
                    if (tool.object.equalsObject(array[i], obj)) {
                        _index = i;
                        break;
                    }
                }
            ...
            sortByDate:function(array, field, desc){
                let _arr = tool.object.cloneObj(array);
        ...
        removeItems:function(array, items){
            ...
            items.map(function (item) {
                if (typeof item == "object") {
                    tool.arrayServer._private.removeSingleObject(array, item);
                }
                if (typeof item == "string") {
                    tool.arrayServer._private.removeSingleStr(array, item);
                }
            })
            return array;
        },
        sortByField:function(array,field,desc){
            ...
            //check Date
            if(!isNaN(tmp)){
                res = tool.arrayServer._private.sortByNumAndChar(array, field, desc);
            }else{
                if (new Date(tmp) != "Invalid Date") {
                    res = tool.arrayServer._private.sortByDate(array, field, desc);
                } else {
                    res = tool.arrayServer._private.sortByNumAndChar(array, field, desc);
                }
            }
    ...
    cookie:{
        removeCookie(name){
            tool.cookie.setCookie(name,"",new Date(0));
        },
        getCookie(name){
